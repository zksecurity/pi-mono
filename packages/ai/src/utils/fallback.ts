 ...
 *
 * `pending` is the in-flight state carried by partial messages, so a *final*
 * message holding it means the attempt ended without ever reaching a terminal
 * stop reason. Report that as an error rather than passing it off as a
 * successful turn, and give it an errorMessage so the retry and refusal
 * classifiers have something to read.
 */
		...
		default:
			return {
				...
				error: {
					...message,
					stopReason: "error",
					errorMessage: message.errorMessage ?? "Stream ended without a terminal stop reason",
				},
			};
	}
}
