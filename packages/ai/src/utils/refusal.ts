 ...
 * `mapStopReason` in `api/anthropic-messages.ts` gives it an explicit
 * errorMessage ("...refused to complete the request (content flagged as
 * sensitive)") so the `/refused to complete the request/i` pattern matches it.
 * Before that it produced `stopReason: "error"` with no message at all and
 * surfaced to users as "An unknown error occurred", invisible to any classifier.
 */
